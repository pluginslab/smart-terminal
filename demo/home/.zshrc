...
# `claude` is the demo's fake (demo/fake-claude.swift, built by scripts/demo-screenshot.sh).
...
claude() {
    ...
    [[ -x "$fake" ]] || { print -u2 "demo: fake claude not built (run scripts/demo-screenshot.sh)"; return 1; }
    "$fake" "$@"
}
